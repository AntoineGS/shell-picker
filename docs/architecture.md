...
Fzf renders a two-line layout with a bounded mode-only prompt and a separate location header. On Linux, startup and resize invoke the typed `d` display callback; picker-specific `i:cd` and `i:cp` commands render the exact custom info-command counts and transforms. On Windows, the native `--info=inline-right` layout is used and the info/display startup callbacks are omitted, so count formatting and placement differ. The display callback reads the current actor snapshot without applying a transition, changing generation, rebuilding candidates, or affecting selection. Navigation effects carry the full escaped location, and the terminal adapter emits the final width-aware `change-header` action.
...
Both navigation and restore reloads use fzf's `wait` barrier before accepting further input. Without it, rapid query edits can replace a queued reload request before fzf starts its load callback, leaving the coordinator waiting indefinitely for load finalization. Only navigation additionally moves the cursor to the first result; restore does not force that repositioning.
...
The fzf listen sidecar is experimental, disabled by default, and enabled only by explicit opt-in. No adapter injects it. Callback opcode grammar remains unchanged and authoritative for the non-sidecar path.
