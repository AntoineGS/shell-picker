...
zoxide returns absolute filesystem paths. Arbitrary legal bytes are accepted only in absolute filesystem paths; a relative row is a malformed soft failure for the whole buffer, and a virtual token never reaches this validation. The caller cause wins over a later private timeout, leaving no partial records and a process reaped. Cached/fresh and timeout settings apply only to the eligible launch query that can populate the initial CD view; fresh plus timeout zero is authoritative only for that query. Direct concurrent initial fresh calls remain protected by the session's cancellation-aware permit, a cancelled waiter returns promptly without factory or attempt, and there is no package-global mutex across independent sessions. Every later navigation generation, including a return to the launch path, is local-only and reports zoxide_outcome `not-run` with zero zoxide counters. `cp` never invokes zoxide. Missing/spawn failure on the launch query is attempt 1/start 0; counters assert attempts, starts, exits, processes, and max-live.
