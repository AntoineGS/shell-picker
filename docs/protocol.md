...
@numeric_bounds,candidate_count=0..1000000|child_starts=0..3|duration_us=0..86400000000|generation=0..18446744073709551615|max_live_children=0..1<=child_starts|schema=1|timestamp=required-rfc3339nano-utc|timestamp_input=zero-or-now-24h0m0s..now+1s|zoxide_attempts=0..1000000|zoxide_exits=zoxide_starts|zoxide_live=0|zoxide_max_live=0..zoxide_starts|zoxide_processes=zoxide_starts|zoxide_starts=0..zoxide_attempts,,,,
...
@zoxide_outcomes,cached|cancelled|malformed|missing|not-run|ok|process-error|timeout,,,,
...
The machine table is the validator contract. `@renderers`, `@zoxide_policies`, and `@zoxide_outcomes` are the exact accepted vocabularies. Every preview event requires a renderer, while every non-preview event requires an empty renderer. A generation is an unsigned 64-bit value and may be nonzero only for generation start/publish/discard and callback load; zero remains validator-valid for those events. Candidate count is 0..1,000,000 and may be nonzero only for generation publish; path also applies only to generation publish. Every recorded `time` is required RFC3339Nano UTC. A caller may omit the input timestamp to use the current time, or supply one inclusively bounded from 24 hours before validation through one second after it.

All six input durations are bounded from zero through 24 hours inclusive and are emitted as microseconds from 0 through 86,400,000,000. Generation timings apply only to publish/discard, callback IPC only to callback event, and load timing only to callback load. The `zoxide_*` bundle means policy plus outcome and may include zoxide counters/duration; it occurs only on generation publish/discard. Zoxide counters are nonnegative; attempts are at most 1,000,000 and bound starts; exits and processes equal starts; live is zero; and `zoxide_max_live` is 0..starts. Child counters are always zero except on preview.finished and preview.exit; there child starts are 0..3, max live is 0..1 and never exceeds starts, and native remains 0/0.
