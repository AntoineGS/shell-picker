...
type EventFinalizeRequest struct {
	EventID uint64 `json:"event_id"`
	Applied bool   `json:"applied"`
}

type LoadFinalizeRequest struct {
	EventID uint64 `json:"event_id"`
	Applied bool   `json:"applied"`
}

// EventResult is the backend result for one authenticated event. EventID is
// zero for backends without a coordinator; nonzero IDs are opaque coordinator
// reservations that must be finalized by the callback client.
...
type Backend interface {
	HandleEvent(context.Context, protocol.Event) (EventResult, error)
	LoadGeneration(context.Context, LoadRequest) ([]byte, error)
...
// EventFinalizer is an optional backend hook invoked after the event response
// has been written. It lets a coordinator defer destructive stream cleanup
// until fzf has received the action generated by the callback.
type EventFinalizer interface {
	FinalizeEvent(context.Context, EventFinalizeRequest) error
}

type LoadFinalizer interface {
	FinalizeLoad(context.Context, LoadFinalizeRequest) error
}
