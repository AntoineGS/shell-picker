...
| --- | --- | --- |
| Insert | `enter:transform(e:en)`<br>`esc:transform(e:es)`<br>`ctrl-l,tab,right:transform(e:fw)`<br>`ctrl-h,left:transform(e:up)`<br>`/:transform(e:sl)`<br>`~:transform(e:hm)`<br>`change:transform(e:rs)`<br>`result-final:rebind(change)+unbind(result-final)` | `enter:transform(e:en)`<br>`esc:transform(e:es)`<br>`ctrl-l,tab,right:transform(e:fw)`<br>`ctrl-h,left:transform(e:up)`<br>`/:transform(e:sl)`<br>`~:transform(e:hm)`<br>`change:transform(e:rs)`<br>`result-final:rebind(change)+unbind(result-final)` |
| Normal | `enter:transform(e:en)`<br>`esc:transform(e:es)`<br>`i:transform(e:mi)`<br>`a:transform(e:ma)`<br>`ctrl-l,tab,right:transform(e:fw)`<br>`ctrl-h,left:transform(e:up)`<br>`/:transform(e:sl)`<br>`~:transform(e:hm)`<br>`j:down`<br>`k:up`<br>`h:trigger(ctrl-h)`<br>`l:trigger(tab)`<br>`q:abort`<br>`ctrl-u:half-page-up`<br>`ctrl-d:half-page-down`<br>`,:preview-half-page-up`<br>`.:preview-half-page-down`<br>`change:transform(e:rs)`<br>`result-final:rebind(change)+unbind(result-final)`<br>`space:clear-multi+toggle`<br>`other printable keys:ignore` | `enter:transform(e:en)`<br>`esc:transform(e:es)`<br>`i:transform(e:mi)`<br>`a:transform(e:ma)`<br>`ctrl-l,tab,right:transform(e:fw)`<br>`ctrl-h,left:transform(e:up)`<br>`/:transform(e:sl)`<br>`~:transform(e:hm)`<br>`j:down`<br>`k:up`<br>`h:trigger(ctrl-h)`<br>`l:trigger(tab)`<br>`q:abort`<br>`ctrl-u:half-page-up`<br>`ctrl-d:half-page-down`<br>`,:preview-half-page-up`<br>`.:preview-half-page-down`<br>`change:transform(e:rs)`<br>`result-final:rebind(change)+unbind(result-final)`<br>`space:toggle`<br>`other printable keys:ignore` |
| Add | `enter:transform(e:en)`<br>`esc:transform(e:es)`<br>`change:transform(e:rs)`<br>`result-final:rebind(change)+unbind(result-final)` | `enter:transform(e:en)`<br>`esc:transform(e:es)`<br>`change:transform(e:rs)`<br>`result-final:rebind(change)+unbind(result-final)` |

Normal `i` enters Insert and Normal `a` enters Add. Normal Esc intentionally aborts the picker rather than applying the legacy clear-multi behavior. Insert keeps the static Enter/Esc callbacks, the actual navigation bindings, and the transient restore handoff shown above; it does not bind `i`. Add has no normal or navigation bindings: Enter creates and loads the proposed directory tree rather than accepting picker output, while Esc returns to Normal. The `result-final`/`change` pair is a one-shot internal handoff: invalid rendering arms `change` only after `result-final`, and the next edit restores the authenticated current generation without rebuilding candidates or changing session state.
