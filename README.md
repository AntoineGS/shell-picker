...
`make build` writes bin/shell-picker, embeds the current Git commit as its development version, and updates the pinned commit in `~/gits/configurations/Both/ShellPicker/setup-shell-picker.sh` when that file exists. Override `SHELL_PICKER_SETUP_SCRIPT` to use another path. `make install` embeds the same commit and uses Go's `GOBIN`, or the default `GOPATH/bin` when `GOBIN` is unset. Go 1.26.5 and external fzf 0.74.1 remain prerequisites.
...
`zoxide query --list --all` returns absolute filesystem paths directly from the database without checking whether every directory is currently available. This keeps stale network-mount entries from blocking picker startup. Accepted rows preserve arbitrary legal bytes in a valid platform-absolute path; a relative row is a malformed soft failure for the whole buffer, and virtual tokens never reach this check. Timeout and partial output are discarded.
